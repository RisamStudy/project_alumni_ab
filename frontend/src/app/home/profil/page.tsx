...
const resolvePhotoURL = (url?: string) => {
  ...
  if ((url.startsWith("http://") || url.startsWith("https://")) && API_BASE_URL) {
    try {
      const parsed = new URL(url);
      if (parsed.pathname.startsWith("/uploads/")) return `${API_BASE_URL}${parsed.pathname}`;
    } catch {
      return url;
    }
  }
...
export default function ProfilPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaved(true);
      setTimeout(() => setSaved(false), 3002);
    } finally {
      setSaving(false);
    }
  };
