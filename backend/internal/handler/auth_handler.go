...
func (h *AuthHandler) UploadProfilePhoto(w http.ResponseWriter, r *http.Request) {
	...
	photoURL := buildPublicUploadURL(filename)
...
func buildPublicUploadURL(filename string) string {
	baseURL := strings.TrimSpace(os.Getenv("PUBLIC_BASE_URL"))
	if baseURL != "" {
		return fmt.Sprintf("%s/uploads/profiles/%s", strings.TrimRight(baseURL, "/"), filename)
	}
	// Default to a relative URL to avoid domain drift behind reverse proxies.
	return fmt.Sprintf("/uploads/profiles/%s", filename)
}
