...
func main() {
	...
	fmt.Println("=== Create Super Admin Account ===")
	email := ask(reader, "Masukkan email super admin")
	if !isValidEmail(email) {
		log.Fatal("Email tidak valid")
	}

	password := ask(reader, "Masukkan password super admin (minimal 8 karakter)")
	if len(password) < 8 {
		log.Fatal("Password minimal 8 karakter")
	}

	confirm := ask(reader, "Konfirmasi password super admin")
	if password != confirm {
		log.Fatal("Konfirmasi password tidak cocok")
	}

	fullName := askWithDefault(reader, "Masukkan nama super admin", "Super Admin Portal")
	...
	userID, created, err := upsertSuperAdmin(dbConn, fullName, int16(birthYear), strings.ToLower(email), string(hash))
	if err != nil {
		log.Fatalf("Gagal membuat akun super admin: %v", err)
	}
	...
	if created {
		fmt.Println("Akun super admin berhasil dibuat")
	} else {
		fmt.Println("Akun sudah ada, data diperbarui dan dijadikan super admin")
	}
	fmt.Printf("Email: %s\n", email)
...
func upsertSuperAdmin(dbConn *sql.DB, fullName string, birthYear int16, email, passwordHash string) (string, bool, error) {
	var userID string
	...
	if err == sql.ErrNoRows {
		if _, err := dbConn.Exec(
			`INSERT INTO users (id, full_name, birth_year, email, password, status, role)
			 VALUES (UUID(), ?, ?, ?, ?, 'active', 'super_admin')`,
	...
	if _, err := dbConn.Exec(
		`UPDATE users
		 SET full_name = ?, birth_year = ?, password = ?, status = 'active', role = 'super_admin'
		 WHERE id = ?`,
